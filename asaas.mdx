# Headers de Canal Asaas - Configuração
...
⚠️ **Modificação do Vendor**: O arquivo `vendor/leopaulo88/asaas-sdk-laravel/src/Support/AsaasClient.php` foi modificado. 

**Recomendações**:
- Considere fazer um fork do pacote e usar sua versão
- Ou use patches do Composer (`cweagans/composer-patches`)
- Documente esta modificação para atualizações futuras do pacote
...
**Problema Identificado**: A detecção de tenant via `session('loja_id')` pode falhar em contextos CLI/queue, e o fallback para subdomínio podia extrair dados incorretos de domínios customizados de clientes.
...
**Problema**: Nomes de variáveis ambíguos que podiam causar confusão.

**Solução**: Nomenclatura mais clara e consistente:
...
**Objetivo**: Confirmar que headers estão sendo aplicados corretamente em produção.

**Implementação**: Debug logging temporário no `AsaasClient.php`:
...
O sistema está agora robusto e pronto para uso em produção multi-tenant com todas as considerações de segurança implementadas.
